...
// 채널 이미지 클릭하면 채널 페이지로 이동
    // JavaScript (index.html)
    // 클릭된 이미지의 채널 정보를 가져와서 channel.html로 이동하는 함수
    function openChannel(imageElement) {
      // 클릭된 이미지의 data-channel 속성에 저장된 채널 이름을 가져옵니다.
      const channelName = imageElement.dataset.channel;

      // channel.html로 이동하는 링크를 생성합니다.
      const channelLink = `test_channel.html?channel=${encodeURIComponent(channelName)}`;

      // 현재 페이지를 channel.html로 이동합니다.
